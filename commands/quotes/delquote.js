...
module.exports.run = async (ts, ev, client, args) => {
  if (!args[0]) return ts.sendTextMessage(client.clid, 1, 'Missing argument!');
  const num = parseInt(args[0], 10);

  const quote = await ts.data.collection('quotes').findOne({ number: num });
  if (!quote) return ts.sendTextMessage(client.clid, 1, 'That quote could not be found.');
  ...
  ts.data.collection('quotes').deleteOne({ number: num }, (err, res) => {
    ...
    if (res.result.ok === 1) {
      ...
      ts.sendTextMessage(client.clid, 1, `Successfully deleted quote ${num}.`);
    }
  });
};
