...
function delUser (ts, client, identifier) {
  if (ipRegex.test(identifier)) {
    ts.data.collection('whitelist').findOneAndDelete({ ip: identifier }).then(res => {
      if (res.value) return ts.sendTextMessage(client.getID(), 1, `IP "${identifier}" removed from whitelist.`);
      return ts.sendTextMessage(client.getID(), 1, `IP "${identifier}" is not whitelisted.`);
    });
  } else if (mytsRegex.test(identifier)) {
    ts.data.collection('whitelist').findOneAndDelete({ mytsid: identifier }).then(res => {
      if (res.value) return ts.sendTextMessage(client.getID(), 1, `MYTSID "${identifier}" removed from whitelist.`);
      return ts.sendTextMessage(client.getID(), 1, `MYTSID "${identifier}" is not whitelisted.`);
    });
  }
  ts.data.collection('whitelist').findOneAndDelete({ name: identifier }).then(res => {
    if (res.value) return ts.sendTextMessage(client.getID(), 1, `User "${identifier}" removed from whitelist.`);
    return ts.sendTextMessage(client.getID(), 1, `User "${identifier}" is not whitelisted.`);
  });
}
...
module.exports.run = (ts, ev, client, args) => {
  ...
  if (args[0].toLowerCase() === 'add' && !args[2]) {
    return ts.sendTextMessage(client.getID(), 1, 'error: Missing argument(s)!');
  }
  ...
  switch (args[0].toLowerCase()) {
    ...
    case 'enable':
      ...
      ts.sendTextMessage(client.getID(), 1, 'Server whitelist enabled.');
      break;
    case 'disable':
      cfg.modules.whitelist = false;
      ts.sendTextMessage(client.getID(), 1, 'Server whitelist disabled.');
      break;
    case 'list':
      ...
      break;
    default:
      return ts.sendTextMessage(client.getID(), 1, 'Didn\'t recognize that subcommand!');
  }
};
