...
module.exports.run = async (ts, ev, client, args) => {
  ...
  ts.data.collection('users').updateOne(filter, update, (err, res) => {
    ...
    if (res.result.n === 0) {
      ...
      ts.sendTextMessage(client.clid, 1, `${args[0]} is already an elevated user.`);
    } else if (res.result.nModified === 1) {
      ts.sendTextMessage(client.clid, 1, `Successfully elevated ${args[0]}.`);
      log.info(`[DB] User "${args[0]}" elevated by "${client.nickname}" (DBID ${client.databaseId})`);
    } else {
      ts.sendTextMessage(client.clid, 1, 'Issue elevating user.');
    }
  });
};
...
module.exports.info = {
  name: 'promote',
  usage: `${cfg.bot.prefix}promote <name>`,
